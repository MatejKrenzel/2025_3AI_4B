...
export class UserCardComponent implements OnInit {
  ...
  upvote() {
    if (this.userPost.id) {
      ...
      this.firestoreService.updatePostField(this.userPost.id, 'upvotes', this.userPost.upvotes)
        .catch((error: any) => console.error("Error upvoting post:", error));
    }
  }
  ...
  downvote() {
    if (this.userPost.id) {
      ...
      this.firestoreService.updatePostField(this.userPost.id, 'downvotes', this.userPost.downvotes)
        .catch((error: any) => console.error("Error downvoting post:", error));
    }
  }
  ...
  addComment() {
    if (this.newCommentText.trim() && this.userPost.id) {
      ...
      // Ak chceš reálne pridávať komentáre do Firestore, odkomentuj:
      /*
